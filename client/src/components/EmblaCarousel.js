...
const EmblaCarousel = ({ slides, options = { loop: false } }) => { 
  ...
  const scrollNext = useCallback(() => { // scrollNext is a function that will be called when the next button is clicked
    if (!emblaApi) return;
    emblaApi.scrollNext();
    autoplay.current.reset();
  }, [emblaApi]);

  const scrollPrev = useCallback(() => { // scrollPrev is a function that will be called when the previous button is clicked
    if (!emblaApi) return;
    emblaApi.scrollPrev();
    autoplay.current.reset();
  }, [emblaApi]);

  const onSelect = useCallback(() => { // onSelect is a function that will be called when the user scrolls the carousel
    if (!emblaApi) return; 
    setPrevBtnEnabled(emblaApi.canScrollPrev()); 
    setNextBtnEnabled(emblaApi.canScrollNext());
  }, [emblaApi]);

  useEffect(() => { // useEffect is a function that will be called when the component mounts
    if (!emblaApi) return;
    onSelect();
...
// Styling components
// Embla Container is a hero viewport with a background image centered and scaled to fit always centered in the viewport
// fix bug: make sure the container is always centered on page load 
